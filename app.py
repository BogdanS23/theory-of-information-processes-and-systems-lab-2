...
c = {
    'C₁*': Attr('Нормализация потерь от заболеваемости', 1),
    'C₂*': Attr('Нормализация потерь сельского хозяйства', 1),
    'C₃*': Attr('Нормализация потерь от изменения среды', 1),
    'C₄*': Attr('Нормализация потерь качества жизни', 1),
    'C₅*': Attr('Нормализация потерь предприятия', 1),
    'C₆*': Attr('Нормализация износа оборудования', 1),
    'C₇*': Attr('Нормализация кредитных ресурсов', 1),
    'C₈*': Attr('Нормализация инвесторов', 1),
    'C₉*': Attr('Нормализация спроса на продукцию', 1),
    'C₁₀*': Attr('Нормализация найма сотрудников', 1),
    'C₁₁*': Attr('Нормализация репутации', 1),
    'C₁₂*': Attr('Нормализация уровня смога', 1),
    'C₁₃*': Attr('Нормализация задымленности', 1),
    'C₁₄*': Attr('Нормализация антициклона', 1)
}
