...
v0 = {
    'Cf₁': Attr('Потери от роста заболеваемости населения', 0.8),
    'Cf₂': Attr('Потери сельского хозяйства от воздействия поллютантов', 0.7),
    'Cf₃': Attr('Потери от изменения природной среды', 0.83),
    'Cf₄': Attr('Потери из-за ухудшения качества жизни населения', 0.72),
    'Cf₅': Attr('Потери предприятия от регулирования выбросов', 0.6),
    'Cf₆': Attr('Износ технологического оборудования', 0.52),
    'Cf₇': Attr('Возможность использования кредитных ресурсов', 0.48),
    'Cf₈': Attr('Привлечение зарубежных инвесторов', 0.22),
    'Cf₉': Attr('Спрос на продукцию предприятия', 0.27),
    'Cf₁₀': Attr('Сложность найма сотрудников', 0.54),
    'Cf₁₁': Attr('Деловая репутация компании', 0.2),
    'Cf₁₂': Attr('Повышенный уровень смога', 0.4),
    'Cf₁₃': Attr('Задымленность от лесных пожаров', 0.32),
    'Cf₁₄': Attr('Продолжительный антициклон', 0.14)
}

# Коэффициенты нормализации (адаптированы под экологические нормативы)
c = {
    'Cf₁*': Attr('Нормализация потерь от заболеваемости', 1),
    'Cf₂*': Attr('Нормализация потерь сельского хозяйства', 1),
    'Cf₃*': Attr('Нормализация потерь от изменения среды', 1),
    'Cf₄*': Attr('Нормализация потерь качества жизни', 1),
    'Cf₅*': Attr('Нормализация потерь предприятия', 1),
    'Cf₆*': Attr('Нормализация износа оборудования', 1),
    'Cf₇*': Attr('Нормализация кредитных ресурсов', 1),
    'Cf₈*': Attr('Нормализация инвесторов', 1),
    'Cf₉*': Attr('Нормализация спроса на продукцию', 1),
    'Cf₁₀*': Attr('Нормализация найма сотрудников', 1),
    'Cf₁₁*': Attr('Нормализация репутации', 1),
    'Cf₁₂*': Attr('Нормализация уровня смога', 1),
    'Cf₁₃*': Attr('Нормализация задымленности', 1),
    'Cf₁₄*': Attr('Нормализация антициклона', 1)
}
